...
public enum CharacterSpecies: String, CaseIterable, Codable, Sendable { case human = "Human", dwarf = "Dwarf", halfling = "Halfling", orc = "Orc" }
...
public struct CharacterCreationDraft: Codable, Hashable, Sendable {
    ...
    private mutating func normalizeDependentSelections() {
        ...
        if characterClass == .wizard {
            ...
            spellbook = []
            preparedSpells = choices(preparedSpells, preferred: defaults.preparedSpells, allowed: availableLevelOneSpells, count: characterClass == .cleric ? 4 : 0)
        }
    ...
    public var requiredClassSkillCount: Int { characterClass == .rogue ? 4 : 2 }
    public var availableClassSkills: [String] { Self.skills(for: characterClass) }
    public var requiredCantripCount: Int { characterClass == .wizard ? 3 : characterClass == .cleric ? (divineOrder == .thaumaturge ? 4 : 3) : 0 }
    ...
    public var equippedArmor: String? {
        ...
        switch characterClass { case .fighter: return equipmentChoice == .packageA ? "Chain Mail" : "Studded Leather Armor"; case .rogue: return "Leather Armor"; case .cleric: return "Chain Shirt"; case .wizard: return nil }
    }
    ...
    public var maximumHitPoints: Int { (characterClass == .fighter ? 10 : characterClass == .wizard ? 6 : 8) + modifier(.constitution) + (species == .dwarf ? 1 : 0) }
    public var speed: Int { equippedArmor == "Chain Mail" && finalScores[.strength, default: 8] < 13 ? 20 : 30 }
    public var size: String { species == .halfling || (species == .human && humanSmall) ? "Small" : "Medium" }
    public var savingThrowProficiencies: [SRD521Ability] { switch characterClass { case .fighter: [.strength, .constitution]; case .rogue: [.dexterity, .intelligence]; case .wizard: [.intelligence, .wisdom]; case .cleric: [.wisdom, .charisma] } }
    ...
    public var allLanguages: [String] { ["Common"] + languages + (characterClass == .rogue ? ["Thieves’ Cant", rogueExtraLanguage] : []) }
    ...
    public var validationErrors: [String] {
        ...
        } else if !expertise.isEmpty { errors.append("Only Rogue has skill Expertise at level one in these four classes.") }
        ...
        if characterClass == .wizard {
            ...
            if preparedSpells.count != 4 || Set(preparedSpells).count != 4 || !Set(preparedSpells).isSubset(of: Set(availableLevelOneSpells)) { errors.append("Prepare four different level-one Cleric spells.") }
        } else if !spellbook.isEmpty || !preparedSpells.isEmpty { errors.append("This class has no class spellcasting at level one.") }
        if let originClass = background.spellClass {
    ...
    public func build() throws -> OpenWorldHero {
        ...
        var hero = OpenWorldHero(name: name.trimmingCharacters(in: .whitespacesAndNewlines), characterClass: characterClass, scores: finalScores, skills: Dictionary(uniqueKeysWithValues: trainedSkills.map { ($0, expertise.contains($0) ? 4 : 2) }), hitPoints: maximumHitPoints, maximumHitPoints: maximumHitPoints, armorClass: armorClass, spellSlots: [.wizard, .cleric].contains(characterClass) ? 2 : 0, secondWindUses: characterClass == .fighter ? 2 : 0, weapons: startingWeapons, spells: cantrips + preparedSpells, equipment: startingEquipment)
    ...
    public var manualAdjudicationNotes: [String] {
        var notes = ["Character creation follows the listed SRD choices. Full class and species trait resolution is not yet automated."]
    ...
    public static func suggested(for cls: AdventurerClass, name: String = "") -> Self {
        ...
        draft.background = cls == .fighter ? .soldier : cls == .rogue ? .criminal : cls == .wizard ? .sage : .acolyte
        draft.method = .standardArray
        let scores: [Int] = cls == .fighter ? [15, 14, 13, 8, 10, 12] : cls == .rogue ? [12, 15, 13, 14, 10, 8] : cls == .wizard ? [8, 12, 13, 15, 14, 10] : [14, 8, 13, 10, 15, 12]
        draft.baseScores = Dictionary(uniqueKeysWithValues: zip(Self.abilities, scores))
        draft.backgroundBoosts = cls == .fighter ? [.strength: 2, .constitution: 1] : cls == .rogue ? [.dexterity: 2, .constitution: 1] : cls == .wizard ? [.intelligence: 2, .constitution: 1] : [.wisdom: 2, .charisma: 1]
...
public extension OpenWorldHero {
    var spellcastingAbility: SRD521Ability? { characterClass == .wizard ? .intelligence : characterClass == .cleric ? .wisdom : nil }
